...
function App() {
  ...
  const getTemperaturesList = () => {
    const listXY = data.filter(register => register.topic === 'temperatura').map( temp => {
      const date = new Date(temp.createdAt)
      return temp = { "x": temp.value, "y": date.getHours()}
    })
    const tempLine = {
      "id": "Temperatura",
      "color": "hsl(47, 70%, 50%)",
  ...
  return (
    <div className="App">
      ...
      { isLoading 
        ...
        : (
          <div className="chart">
            <TemperatureGraph list={getTemperaturesList()} />
          </div>
          )
      } 
		</div>
